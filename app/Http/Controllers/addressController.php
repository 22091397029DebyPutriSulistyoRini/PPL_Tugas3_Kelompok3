...
class AddressController extends Controller
{
    ...
    {
        $contact = Contact::where('id_user', $user->id)->where('id', $idContact)->first();
        if (!$contact) {
    ...
    {
        $address = Address::where('id_contact', $contact->id)->where('id', $idAddress)->first();
        if (!$address) {
            throw new HttpResponseException(response()->json([
                'errors' => [
                    "message" => [
    ...
    public function list(int $idContact): JsonResponse
    {
        $user = Auth::user();
        $contact = $this->getContact($user, $idContact);
        $addresses = Address::where('id_contact', $contact->id)->get();
